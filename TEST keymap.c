...
bool get_custom_auto_shifted_key(uint16_t keycode, keyrecord_t *record) {
	switch (keycode) {
		...
		case KC_COLN:
		case KC_DQT:
		case KC_EQL:	// Test case, delete to revert to normal = +
		case U_EQ:
		case U_AD:
		case U_AR:
		case U_ED:
...
void autoshift_press_user(uint16_t keycode, bool shifted, keyrecord_t *record) {
	switch(keycode) {
		...
		case KC_EQL: register_unicodemap((!shifted) ? UC_EQT : UC_EQH); break;	// Test case, delete to revert to normal = +
		case U_EQ: tap_code16((!shifted) ? KC_A : S(KC_A)); break;	//		case U_EQ: register_unicodemap((!shifted) ? UC_EQT : UC_EQH); break;
...
void autoshift_release_user(uint16_t keycode, bool shifted, keyrecord_t *record) {
	switch(keycode) {
		...
		case KC_EQL: break;	// Test case, delete to revert to normal = +
		case U_EQ: break;
		case U_AD: break;
		case U_AR: break;
		case U_ED: break;
